const language = {
  ...
  error1:
    "O token do bot que você inseriu no projeto é incorreto ou as intents do seu bot estão desligadas!",
  error2:
    "Por favor coloque o token no bot no token.js ou no seu .env do seu projeto!",
  loadclientevent: "Evento do cliente carregado",
  embed1:
    "Você deve ter o cargo <@&{djRole}>(DJ) definido nesse servidor para usar esse comando. Usuários sem esse cargo não podem usar o {cmdMAP}",
  ...
  error4:
    "AVISO: Parece que você não escreveu a url da mongodb? Se você não inserir uma url da mongodb válida no arquivo config.js, você não poderá usar o bot.",
  msg13: `🎵 Tocando agora: **{track?.title}** -> Canal: **{queue?.connection.channel.name}** 🎧`,
  msg14:
    "A lista está vazia. Você pode tocar mais um pouco de música, tchau tchau... ✅",
  msg15: "Eu desconectei porque não tinha mais ninguém no meu canal. ❌",
  msg16:
    "Estou tendo problemas em conectar com o canal de voz. ❌ Tipo alguém me desconectou? Eu estou muito triste. 😔",
  ...
  msg32:
    "Está na fora de ouvir músic no seu servidor do Discord com uma interface complenta grátis e avançada. Bot de músca que suporta tocar música em várias plataformas que vai fazer seu servidor se sentir especial. Crie seu próprio bot de música: https://github.com/umutxyp/MusicBot",
  ...
  msg118:
    "O seu tempo para usar esse comando expirou, você pode digitar `/playlist list {name}` para usar esse comando de novo.",
  msg119:
    "Use o comando **/play playlist <list-name>** para ouvir essas playlists.\nDigite **/playlist list <list-name>** para ver a música em uma lista.",
  msg120: "Por favor especifique um canal de texto.",
  msg121:
    "<#{channel}> foi adicionado a lista de canais de comando, agora os comandos do bot só podem ser usados nos canais da lista.",
  ...
  msg126:
    "❌ Aqui está a lista de canais que você pode usar comandos nesse servidor: {channel_filter}",
  msg127: "Comando não está definidoCommand is not defined.",
  error7:
    "Por favor tente esse comando novamente mais tarde. Possível bug reportado para os desenvolvedores.",
  msg128:
    "Você me silenciou enquanto a música estava tocando. É por isso que eu parei a música. Se você me desmutar, eu irei continuar. 😔",
  msg129: "toca",
  ...
  msg135:
    "O tempo de reprodução foi definido para {queue.formattedCurrentTime} com sucesso.",
